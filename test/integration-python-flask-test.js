...
const pythonRuntime = 'python2.7';
const pipRuntime = 'pip2.7';
...
let _setUpApplication = function(cb){
	...
	_generateApplication(function() {
		helpers
			...
			.then((tmpDir) => {
			  execRun(pipRuntime + ' install flask --user --upgrade', {cwd: tmpDir}, function(error, stdout, stderr){
...
let _generateApplication = function(cb) {
	const serviceNames = ['watson-conversation'];
