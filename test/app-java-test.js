...
class Options {
  ...
  assertConfig(framework, buildType, service, filePath) {
    ...
    var buildTest = common.test(buildType);

    if(expected.dependencies) {
      expected.dependencies.forEach(dep => {
        var scope = dep.scope || 'compile';     //default scope if not set
  ...
  assertlibertysrc(exists, buildType) {
    ...
    var desc = exists ? 'should ' : 'should not ';
    var buildTest = common.test(buildType);
...
var spring_services = getServices('java-spring');
var liberty_services = getServices('java-liberty');
...
function getServices(subFolder) {
  var root = path.join(__dirname, "..", 'generators');
  var folders = fs.readdirSync(root);
  var services = [];
  folders.forEach(folder => {
    if(folder.startsWith('service-')) {
      var svcpath = path.join(root, folder, 'templates', subFolder);
  ...
  BUILD_TYPES.forEach(buildType => {
    services.forEach(dirname => {
      describe(`java generator : test ${framework}, ${buildType}, ${dirname}   `, function () {
        this.timeout(10000);
        var service = svcHelpers.fromDirName(dirname, optionsBluemix);
        var options = new Options(service, backendPlatform);
...
function testTestService() {
  describe('java generator : test liberty, maven, test service', function() {
    ...
    var bluemixJson = optionsBluemix;
    bluemixJson.test = testService;
    var service = svcHelpers.serviceTest(bluemixJson)
    var options = new Options(service, 'JAVA');
    before(options.before.bind(options));
    var configPath = path.join(__dirname, "..", "test", "resources", "java", "service-test", "templates", 'java-liberty', "config.json.template");
